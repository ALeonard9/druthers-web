...
export function BookSearch() {
  ...
  async function add(b: BookSearchResult, list: 'watchlist' | 'rankings') {
    ...
    // the gesture. Exception: the first book into an empty shelf auto-places
    // at #1 (api#289), so there's nothing left to decide - go to the board.
    if (res.ok && !multiAddMode) {
      if (list === 'watchlist') {
        router.push('/books/to-read');
      } else {
        const tracker = await res.json().catch(() => null);
        if (isAlreadyPlaced(tracker)) {
          router.push('/books/ranking');
        } else {
          router.push(duelHrefFor('books', catalogIdFrom('books', tracker)));
        }
      }
    }
  }
