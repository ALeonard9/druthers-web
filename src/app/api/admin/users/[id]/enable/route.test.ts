...
// Re-enabling an account. The way back from disable.
...
const call = () => POST(new Request('http://localhost/api/admin/users/u-1/enable'), ctx);

describe('/api/admin/users/u-1/enable', () => {
  ...
  it('calls the api with the encoded identifier', async () => {
    ...
    expect(mocks.apiFetch).toHaveBeenCalledWith('/v1/admin/users/u-1/enable', { method: 'POST' });
    expect(response.status).toBe(200);
  });

  it('preserves an upstream refusal rather than flattening it', async () => {
    // A failed enable that reads as success leaves someone locked out while the console shows them active.
