...
// Visibility is the tier machinery the whole social surface rests on, so a
// write that silently fails here is a privacy bug, not a UX one.
...
describe('/api/visibility', () => {
  ...
  it('GET returns the current values', async () => {
    mocks.apiFetch.mockResolvedValue({ handle: 'follower', visibility_profile: 'public' });

    const response = await GET();

    expect(mocks.apiFetch).toHaveBeenCalledWith('/v1/users/me/visibility');
    expect(response.status).toBe(200);
    expect(await response.json()).toEqual({ handle: 'follower', visibility_profile: 'public' });
  });
  ...
  it('PUT forwards the body and returns the saved values', async () => {
    ...
    const response = await PUT(
      new Request('http://localhost/api/visibility', {
        method: 'PUT',
        body: JSON.stringify({ handle: 'follower', visibility_profile: 'public' }),
      }),
    );

    expect(mocks.apiFetch).toHaveBeenCalledWith(
      '/v1/users/me/visibility',
      expect.objectContaining({ method: 'PUT', body: { handle: 'follower', visibility_profile: 'public' } }),
    );
    expect(response.status).toBe(200);
  });
