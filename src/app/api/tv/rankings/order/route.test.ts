...
const put = (body: unknown) =>
  PUT(
    new Request('http://localhost/api/tv/rankings/order', {
      method: 'PUT',
      body: JSON.stringify(body),
    }),
  );

describe('PUT /api/tv/rankings/order', () => {
  beforeEach(() => mocks.apiFetch.mockReset());

  it('forwards the new order verbatim', async () => {
    const order = { show_ids: ['c', 'a', 'b'] };
    mocks.apiFetch.mockResolvedValue([]);

    const response = await put(order);

    expect(mocks.apiFetch).toHaveBeenCalledWith('/v1/users/me/tv-shows/rankings/order', {
      method: 'PUT',
      body: order,
    });
  ...
  it('returns the reordered list the api reports, not the request body', async () => {
    ...
    expect(await (await put({ show_ids: ['c'] })).json()).toEqual([{ id: 'c', rank: 1 }]);
  });
  ...
  it('preserves an upstream rejection rather than reporting a saved order', async () => {
    ...
    const response = await put({ show_ids: ['a'] });
  ...
  it('turns an unknown failure into a 500', async () => {
    ...
    expect((await put({ show_ids: ['a'] })).status).toBe(500);
  });
});
