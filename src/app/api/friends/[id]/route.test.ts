...
// Removing a friend. A delete that reports success while the friendship
// survives leaves friends-only shelves open to someone who was removed.
...
const ctx = { params: Promise.resolve({ id: 'friend-1' }) };
describe('/api/friends/[id]', () => {
  beforeEach(() => mocks.apiFetch.mockReset());

  it('DELETE calls the api and answers 200', async () => {
    mocks.apiFetch.mockResolvedValue({ ok: true });

    const response = await DELETE(new Request('http://localhost/api/friends/[id]'), ctx);

    expect(mocks.apiFetch).toHaveBeenCalledWith('/v1/users/me/friends/friend-1', { method: 'DELETE' });
  ...
  it('DELETE turns an unknown failure into a 500', async () => {
    ...
    expect((await DELETE(new Request('http://localhost/api/friends/[id]'), ctx)).status).toBe(500);
  });
});
