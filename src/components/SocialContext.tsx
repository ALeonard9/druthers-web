...
export function SocialContext({ people }: { people: SocialItemContext[] }) {
  return (
            ...
            <li key={person.handle} className="border-t border-line pt-3 first:border-t-0 first:pt-0">
              <div className="flex flex-wrap items-baseline gap-x-2 gap-y-1">
                ...
                <span className="text-xs text-neutral-500">
                  {person.relationship === 'friend' ? 'Friend' : 'Following'}
                </span>
                {person.rank !== null && (
                  <span className="text-sm text-neutral-300">Ranked #{person.rank}</span>
                )}
                {person.on_watchlist && (
                  <span className="text-sm text-neutral-300">On watchlist</span>
                )}
              </div>
              {person.notes === null ? (
