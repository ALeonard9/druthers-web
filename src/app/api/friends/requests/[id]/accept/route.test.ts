...
// Accepting an incoming request. Both sides count as friends from here, so a
// silent failure leaves friends-only shelves closed to someone the user believes
// they accepted.
...
describe('/api/friends/requests/[id]/accept', () => {
  beforeEach(() => mocks.apiFetch.mockReset());

  it('calls the api and returns the result', async () => {
    mocks.apiFetch.mockResolvedValue({ handle: 'stranger' });

    const response = await PUT(new Request('http://localhost/api/friends/requests/[id]/accept'), ctx);

    expect(mocks.apiFetch).toHaveBeenCalledWith('/v1/users/me/friends/requests/req-1/accept', { method: 'PUT' });
  ...
  it('turns an unknown failure into a 500', async () => {
    ...
    expect(
      (await PUT(new Request('http://localhost/api/friends/requests/[id]/accept'), ctx)).status,
    ).toBe(500);
  });
});
