...
describe('GET /api/tv/search', () => {
  ...
  it('rejects a missing query without calling the api', async () => {
    const response = await get('http://localhost/api/tv/search');
    expect(response.status).toBe(400);
    expect(mocks.apiFetch).not.toHaveBeenCalled();
  });

  it('rejects a whitespace-only query', async () => {
    const response = await get('http://localhost/api/tv/search?q=%20%20');
    expect(response.status).toBe(400);
    expect(mocks.apiFetch).not.toHaveBeenCalled();
  });
  ...
  it('url-encodes the query rather than interpolating it raw', async () => {
    ...
    await get('http://localhost/api/tv/search?q=' + encodeURIComponent('a&b=c'));

    expect(mocks.apiFetch).toHaveBeenCalledWith('/v1/tv-shows/search?q=a%26b%3Dc');
  });
  ...
  it('returns an empty list as an empty list, not an error', async () => {
    ...
    const response = await get('http://localhost/api/tv/search?q=nothing');
  ...
  it('passes results through unchanged', async () => {
    ...
    const response = await get('http://localhost/api/tv/search?q=a');
