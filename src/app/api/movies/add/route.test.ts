...
const post = (body: unknown) =>
  POST(new Request('http://localhost/api/movies/add', {
    method: 'POST',
...
describe('POST /api/movies/add', () => {
  ...
  it('creates the catalog row then tracks it, defaulting to the watchlist', async () => {
    ...
    const response = await post({ tmdb: 603, title: 'The Matrix' });

    expect(response.status).toBe(201);
    expect(mocks.apiFetch).toHaveBeenNthCalledWith(
      2,
      '/v1/users/me/movies/catalog-1',
  ...
  it('tracks against rankings when asked, not the watchlist', async () => {
    ...
    await post({ tmdb: 603, title: 'The Matrix', list: 'rankings' });
